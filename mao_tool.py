...
# mao_tool.py (V0.5: TIERED MULTIPLIERS & SAFETY GATE)

def mao_calculator(arv: float, repairs: float, wholesale_fee: float = 10000.00) -> str:
    """
    V0.5 UPGRADE: Implements Tiered Multipliers based on ARV 
    and a Safety Gate to prevent low-ball 'Red Flag' offers.
    """
    try:
        # --- 1. THE DYNAMIC MULTIPLIER (The '2023 Formula') ---
        # We adjust the target margin based on the property value tier.
        if arv < 120000:
            target_margin = 0.70
        elif arv < 200000:
            target_margin = 0.80
            ...
            target_margin = 0.85 # The $300k+ Tier

        # --- 2. THE CALCULATION ---
        target_purchase_price = (arv * target_margin) - repairs
        mao = target_purchase_price - wholesale_fee

        # --- 3. THE SAFETY GATE (The 'Architect's' Guardrail) ---
        # If the offer is under $100k, it triggers a manual review flag 
        # to ensure we don't look like a 'scam' or a 'bot' to the owner.
        if mao < 100000:
            return f"RED FLAG: Offer ${mao:,.2f} is under Safety Floor. MANUAL REVIEW REQUIRED."

        return f"Maximum Allowable Offer (MAO) [{int(target_margin*100)}% Tier]: ${mao:,.2f}"
